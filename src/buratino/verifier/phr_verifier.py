...
from buratino.llm.json_parser import TraceLimits, parse_phr_document_result
from buratino.llm.prompt_loader import PromptLoader
from buratino.models.contracts import DocumentPhrResult, ReasoningTrace
...
class PhrVerifier:
    ...
    def _verify_document(
        ...
        *,
        ...
        index: int,
        total: int,
    ) -> tuple[DocumentPhrResult, DocumentSummary]:
        ...
        logger.info(
            "PHR LLM analysis: document {}/{} file={} source={} mode={}",
            index,
            total,
            document.file_name,
    ...
    def _run_doc_analysis(
        ...
        *,
        ...
        return verify_with_forced_chunks(
            ...
            generate_json=lambda prompt: self._generate_json(prompt_name="phr_fact_summary.md", prompt=prompt, model=llm_model),
    ...
    def _error_result(self, document: DocumentSummary, error_info: JsonStepErrorInfo) -> DocumentPhrResult:
        return DocumentPhrResult(
            document_id=document.document_id,
            file_name=document.file_name,
            phr_fact_status="не подтверждено",
            reasoning="Doc-level PHR analysis failed after JSON repair retries.",
    ...
    def _missing_ocr_result(self, document: DocumentSummary) -> DocumentPhrResult:
        return DocumentPhrResult(
            document_id=document.document_id,
            file_name=document.file_name,
            phr_fact_status="не подтверждено",
            reasoning="OCR отсутствует; документ не анализировался для PHR-подтверждения.",
            reasoning_trace=ReasoningTrace(
                reason_codes=["ocr_missing"],
                missing_requirements=["ocr_text"],
                short_rationale="OCR отсутствует, подтверждение ПХР не проверялось.",
                confidence="low",
            ),
    ...
    def _ocr_analysis_failed_result(self, document: DocumentSummary, error: str) -> DocumentPhrResult:
        return DocumentPhrResult(
            document_id=document.document_id,
            file_name=document.file_name,
            phr_fact_status="не подтверждено",
            reasoning="OCR анализ ПХР не дал подтверждения из-за ошибки chunk processing.",
            reasoning_trace=ReasoningTrace(
                reason_codes=["ocr_chunk_analysis_failed"],
                missing_requirements=["stable_ocr_chunk_analysis"],
                short_rationale="OCR chunk analysis для ПХР завершился ошибкой.",
                confidence="low",
            ),
